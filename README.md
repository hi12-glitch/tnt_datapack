...
- Make sure you ran `/reload` after copying files.
- Make sure `pack.mcmeta` and `data/` are at the datapack root (not nested one extra folder deep).
- This pack now includes both legacy `functions/` and newer `function/` data paths for version compatibility.
